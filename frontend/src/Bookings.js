...
function Bookings() {
    ...
    const handleBookingSubmit = async (e) => {
        ...
        try {
            ...
            const data = await response.json();
            if (response.ok) {
                setBookingConfirmation('Booking Successful! The booking has been saved. \n\nBooking Details:\n' + `Name: ${effectiveUserName}\n` + `Date: ${bookingDate}\n` + `Time: ${selectedTime}\n` + `Room: ${selectedRoom.room}`);
                setSelectedRoom(null);
                if (!currentUser) {
                    setBookingName("");
                }
        ...
        <div style={{ padding: "20px" }}>
            ...
            <div>
                ...
                {selectedFloor === "1" ? (
                    ...
                    <p>No available rooms on Floor {selectedFloor} at {selectedTime}.</p>
                ) : (
                    <ul style={{ listStyleType: "none", padding: 0 }}>
                        {availableRooms.map((room, idx) => (
                            <li
                                key={idx}
                                style={{ marginBottom: "5px", border: "1px solid #ccc", borderRadius: "5px", padding: "10px", cursor: "pointer" }}
            ...
            {selectedRoom && (
                <div style={{ marginTop: "20px", padding: "15px", border: "1px solid #aaa", borderRadius: "5px" }}>
                    <h2>Book {selectedRoom.room}</h2>
                    <p>Floor: {selectedFloor} | Date: {bookingDate} | Time: {selectedTime}</p>
